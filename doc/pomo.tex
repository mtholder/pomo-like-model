...
\subsection*{Summarizing $Q$ matrix caluclations}
\begin{itemize}
...
Because this coarse binning breaks the tight connections with the population genetics underpinnig, one could estimate an extra parameter to generate the state frequencies, and $Q$ matrix:
\begin{compactitem}
    ...
    \item a new frequency parameter, $\psi = \pi_{IJ}^{(N/2)}/\pi_{IJ}^{(N-1)} = \pi_{IJ}^{(N/2)}/\pi_{IJ}^{(1)}$; This govens the relative abundance of the intermediate frequency bin states. and 
